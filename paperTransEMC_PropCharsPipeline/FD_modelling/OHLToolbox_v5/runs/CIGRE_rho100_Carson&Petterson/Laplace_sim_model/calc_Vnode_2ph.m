function [V]=calc_Vnode_2ph(ord,f,Ybranch,Vo1,Vo2)
...
for k=1:1:max(size(f))
    ...
    Ired=-Ybranch_dis_f(3:length(Ybranch_dis_f),1)*Vo1(k)-Ybranch_dis_f(3:length(Ybranch_dis_f),2)*Vo2(k);
    Yred=Ybranch_dis_f(3:length(Ybranch_dis_f),3:length(Ybranch_dis_f)); % Υπολογισμός του πίνακα Υred για τη συγκεκριμένη συχνότητα φάσματος - ((2*ord)-1 x (2*ord)-1)
    ...
    Vred=Yred\Ired;
    Vdis=[Vo1(k);Vo2(k);Vred]; % Αποθήκευση όλων των τάσεων των κόμβων για τη συγκεκριμένη συχνότητα φάσματος - (2*ord x 1)
