function Ym_Petterson = pettersonMutualAdmittance(f,hi,hj,x,rho,e_rg)
...
%
% OUTPUT: Ym_Petterson : mutual admittance between conductors i and j [S/m]
...
n = sqrt(e_rg + (sigma_g/(1i*w*e0)));

Ne = (1/((n^2 + 1)))*(log(-(sqrt(H + ((n^2 + 1)/(beta))+x^2))/(Dij)));

Peij = 1/(2*pi*e0)*(log(Dij/dij)+Ne);

Ym_Petterson = 1i*w*(Peij)^(-1);
