function [SER,poles,rmserr,fit,opts]=vectfit4(f,s,poles,weight,opts)
%
...
if s(1)==0 && a==1
    ...
    elseif poles(1)==0 && poles(2)==0
        poles(1)=-1+1i*10; poles(2)=-1-1i*10;
    end
end
...
if opts.skip_pole~=1
    ...
    for m=1:N
        ...
        elseif cindex(m)==1  %complex pole, 1st part
            ...
            Dk(:,m+1)=1i./(s-LAMBD(m,m)) - 1i./(s-LAMBD(m,m)');
        end
    end
    ...
    if opts.spy1==1
        ...
        if opts.logx==1
            ...
            if opts.logy==1
                ...
                figure(3),
                h1=plot(s./(2*pi*1i),abs(RES3'),'b'); xlim([freq(1) freq(Ns)]);
            end
        end
    ...
    cp_poles=imag(roetter)>0;
    cp_poles_conj=imag(roetter)<0;
    if opts.replace_complex_pairs==1
        roetter(cp_poles)=sqrt(real(roetter(cp_poles)).^2+imag(roetter(cp_poles)).^2)+DELTA; %Forcing poles to be real
        roetter(cp_poles_conj)=sqrt(real(roetter(cp_poles_conj)).^2+imag(roetter(cp_poles_conj)).^2)-DELTA;
    end
    ...
    roetter=roetter-2*1i*imag(roetter); %10.11.97 !!!
    SERA=roetter.';
...
if opts.skip_res~=1
    ...
    for m=1:N
        ...
        elseif cindex(m)==1  %complex pole, 1st part
            ...
            Dk(:,m+1)=1i./(s-LAMBD(m)) - 1i./(s-LAMBD(m)');
        end
    end
    ...
    if common_weight==1
        ...
        for m=1:N
            ...
            elseif cindex(m)==1  %complex pole, 1st part
                ...
                Dk(:,m+1)=1i.*weight./(s-LAMBD(m)) - 1i.*weight./(s-LAMBD(m)');
            end
        end
    ...
    for m=1:N
        if cindex(m)==1
            for n=1:Nc
                r1=C(n,m); r2=C(n,m+1);
                C(n,m)=r1+1i*r2; C(n,m+1)=r1-1i*r2;
            end
        end
    end
    % **************
