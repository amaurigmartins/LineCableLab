function [out] = punchJMartiCard(NP, fitZc, fitA1, Ti, fname)
% ESCOPO:
...
for m=1:NP
    polesZc{m,1} = length(fitZc(m).pol);
    polesZc{m,2} = fitZc(m).zInf;
    polesZc{m,3} = fitZc(m).pol';
    resZc{m,1} = length(fitZc(m).res);
    resZc{m,2} = fitZc(m).res';
    
    polesA1{m,1} = length(fitA1(m).pol);
    polesA1{m,2} = fitA1(m).tauInf;
    polesA1{m,3} = fitA1(m).pol';
    resA1{m,1} = length(fitA1(m).res);
    resA1{m,2} = fitA1(m).res';
end
...
for i = 1: NP
    ...
    node1 = sprintf('%s__%s',inTag,seq(i));
    node2 = sprintf('%s__%s',outTag,seq(i));
    ...
    out_infValZc(11:40) = modprin(30,InfVal_Zc(end));
