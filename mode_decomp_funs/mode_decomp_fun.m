function [Zch_mod,Ych_mod,Zch,Ych,g,a,vel,Ti,Z,Y] = mode_decomp_fun(Z_full,Y_full,freq,freq_siz,ord,decmp_flag)
...
elseif decmp_flag == 2
    ...
    % end
    % if (ZYprnt)
    %     plotwavchar_fun(freq,ord,a,vel,Zch_mod,Ti,tag)
    % end
elseif decmp_flag == 3
...
elseif decmp_flag == 12
    ...
    %     plotwavchar_fun(freq,ord,a,vel,Zch_mod,Ti,tag)
    % end
elseif decmp_flag == 13
    [Ti,g]=eigshuf_QR_decomp(ord,freq_siz,Z,Y); % ???? QR decomposition ????? ???????? ??? eigenvector switchover - ?????????? ?????? ??????????????? Ti (current) (num_files x ord^2) ??? ????????? g (sqrt) (num_files x ord)
    % [Zch_mod,Ych_mod,Zch,Ych]=calc_char_imped_admit(Ti,Z,Y,ord,freq_siz);
    % a=real(g);
    % b=imag(g);
    % freq_siz=length(freq);
    ...
    %     plotwavchar_fun(freq,ord,a,vel,Zch_mod,Ti,tag)
    % end
elseif decmp_flag == 14
    [Ti,g]=intercheig_QR_decomp(ord,freq_siz,Z,Y); % ???? QR decomposition ????? ???????? ??? eigenvector switchover - ?????????? ?????? ??????????????? Ti (current) (num_files x ord^2) ??? ????????? g (sqrt) (num_files x ord)
    % [Zch_mod,Ych_mod,Zch,Ych]=calc_char_imped_admit(Ti,Z,Y,ord,freq_siz);
    % a=real(g);
    % b=imag(g);
    % freq_siz=length(freq);
...
% [Ti]=min_imag_part(Ti,ord,freq_siz);
...
Ti=rot_min_imag(Ti,ord,freq_siz); %achived a slightly smaller imag part in comparison to atp_min_imag

%% Compute the actual modal quantities
[Zch_mod,Ych_mod,Zch,Ych]=calc_char_imped_admit(Ti,Z,Y,ord,freq_siz);
a=real(g);
b=imag(g);
freq_siz=length(freq);
