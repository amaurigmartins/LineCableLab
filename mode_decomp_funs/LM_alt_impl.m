function [Ti,g]=LM_alt_impl(ord,num_files,Z,Y,freq)
...
%     k % Δείκτης για να ενημερώνει στο command window σε ποια συχνότητα βρισκόμαστε
    ...
    if k==1 % Στην πρώτη συχνότητα εφαρμόζεται QR decomposition διότι έτσι ξεκινά ο αλγόριθμος και δίοτι είναι τα 0 Hz όπου δεν μπορεί να κανονικοποιηθεί με νόρμα το υπό εξέταση γινόμενο - Προσοχή!!! Χειροκίνητα μπορεί να εφαρμοστεί και στη δεύτερη συχνότητα κ.ο.κ.
        ...
        for o=1:ord
            ...
            obj=@(x); % Συνάρτηση αντικειμένου - Περιέχει το πρόβλημα της διαγωνοποίησης συν δυο περιοριστικές εξισώσεις για το μέτρο των ιδιοδιανυσμάτων! Επιστρέφει ένα πραγματικό διάνυσμα (2*ord x 1)
