function [Zg_self]=Z_papad_slf(h,r,eps1,mu1,sigma1,f,con,kx)

if nargin==7
    kx=0; %sets default to zero
end

% Function for the Self Earth Impedance as per Papadopoulos formula
...
for k=1:1:con
    ...
    if h(1,k) < 0
        ...
        zz=@(a0,a1,hi,hj,lambda,mu0,mu1,omega,y) ...
            (mu1.*omega.*exp(-a1.*abs(hi-hj+1e-3)).*cos(lambda.*y) ...
            .*5.0e-1i)./(a1.*pi)-(mu1.*omega.*exp(-a1.*(hi-hj)).*cos(lambda.*y) ...
            .*(a0.*mu1+a1.*mu0.*sign(hi)).*5.0e-1i)./(a1.*pi.*(a0.*mu1+a1.*mu0));
