function [Ztot_Carson,Ztot_Noda,Ztot_Deri,Ztot_AlDe,Ztot_Sunde,Ztot_Pettersson,Ztot_Semlyen,Ztot_Wise,Ztot_Papad,Ztot_OvUnd,Nph] = Z_clc_fun(f_total,ord,ZYprnt,FD_flag,siz,soil,h,d,Geom,ZYsave,jobid)
%% Variables
...
for k=1:siz
    ...
    elseif all(h<0) % all conductors are underground
        ...
        Zs_papad=Z_papad_slf(h,cab_ex,e_g ,m_g,sigma_g,f,ord,kx);
        Zm_papad=Z_papad_mut(h,d,e_g,m_g,sigma_g,f,ord,kx);
        ...
        kx='k0';
        Zs_papad=Z_papad_slf(h,cab_ex,e_g ,m_g,sigma_g,f,ord,kx); % self impedances of the underground conductors
        Zm_papad=Z_papad_mut(h,d,e_g ,m_g,sigma_g,f,ord,kx); % mutual impedances of the underground conductors
        Zs_kik=Z_kik_slf(h,cab_ex,e_g ,m_g,sigma_g,f,ord,kx); % self impedances of the overhead conductors
        Zm_kik=Z_kik_mut(h,d,e_g ,m_g,sigma_g,f,ord,kx); % mutual impedances of the overhead conductors
        Zm_new=Z_new_mut(h,d,e_g ,m_g,sigma_g,f,ord,0); % mutual impedances in the mixed configuration
...
%% Plot parameters
if (ZYprnt)
    plotZ_fun_ct(f_total,Nph,Ztot_Carson,Ztot_Noda,Ztot_Deri,Ztot_AlDe,Ztot_Sunde,Ztot_Pettersson,Ztot_Semlyen,Ztot_Wise,Ztot_Papad,Ztot_OvUnd,jobid);
end

if (ZYsave)
    fname = fullfile(pwd,'Z_pul_output.mat');
    save(fname,'Ztot_Carson', 'Ztot_Noda', 'Ztot_Deri', 'Ztot_AlDe', 'Ztot_Sunde', 'Ztot_Pettersson', 'Ztot_Semlyen', 'Ztot_Wise','Ztot_Papad','Ztot_OvUnd' );
end