...
for i=1:num_modes
    ...
    for j=1:2
        for k=1:3
            ...
            semilogx(f,abs(zch(:,i)),'DisplayName', [formula{j}(6:9) '__' ds_name{k}(1:end-6) '__rho' rho_names{idx}]);hold all
        end
    end
    title(['mode #' num2str(i)])
    axis tight
    xlabel('Frequency [Hz]')
    ylabel('Z_{ch} magnitude [\Omega]')
    grid on
    legend
end
...
for i=1:num_modes
    ...
    for j=1:2
        for k=1:3
            zch=read_from_file(eval([formula{j} '_' ds_name{k} '_dataset{idx}']),'Zch_mod');
            semilogx(f,radtodeg(angle(zch(:,i))),'DisplayName', [formula{j}(6:9) '__' ds_name{k}(1:end-6) '__rho' rho_names{idx}]);hold all
        end
    end
    title(['mode #' num2str(i)])
    axis tight
...
for i=1:num_modes
    ...
    for j=1:2
        for k=1:3
            ...
            semilogx(f,a(:,i),'DisplayName', [formula{j}(6:9) '__' ds_name{k}(1:end-6) '__rho' rho_names{idx}]);hold all
        end
    end
    title(['mode #' num2str(i)])
    axis tight
...
for i=1:num_modes
    ...
    for j=1:2
        for k=1:3
            ...
            semilogx(f,vel(:,i),'DisplayName', [formula{j}(6:9) '__' ds_name{k}(1:end-6) '__rho' rho_names{idx}]);hold all
        end
    end
    title(['mode #' num2str(i)])
    axis tight
