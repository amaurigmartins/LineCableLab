...
% job parameters
soil_rho=11924; %resistivity of earth - must be DC value
soil_eps=3;
fd_model = 1; %LS model
Zmod_src='Ztot_Pettersson';
Ymod_src='Ytot_Pettersson';
jobid = ['FD' num2str(fd_model) '_' Zmod_src '_' Ymod_src '_rho' strrep(num2str(soil_rho),'.','_') '_eps' num2str(soil_eps)];
...
%% Calculations
tic
[Ztot_Carson,~,~,~,~,Ztot_Pettersson,~] = Z_clc_fun(f,ord,ZYprnt,FD_flag,freq_siz,soil,h,d,Geom,ZYsave,jobid); % Calculate Z pul parameters by different earth approaches
