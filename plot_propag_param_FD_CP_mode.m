...
set_plot_params()
figname=['WiSoilModel_ratio_mode' num2str(mode)]
figure('Name',figname)
...
for idx=1:3
a_ref=read_from_file(const_model_dataset{idx+3},'a_dis');
a=read_from_file(LS_model_dataset{idx+3},'a_dis');
yya = a ./ a_ref;
semilogx(f,yya(:,mode),'DisplayName', ['LS model - \rho = ' rho_names{idx} ' \Omega.m'], 'LineStyle','-');hold all

a_ref=read_from_file(const_model_dataset{idx+3},'a_dis');
a=read_from_file(cigre_model_dataset{idx+3},'a_dis');
yya = a ./ a_ref;
semilogx(f,yya(:,mode),'DisplayName', ['Cigre model - \rho = ' rho_names{idx} ' \Omega.m'], 'LineStyle',':');hold all
end
title(['Normalized attenuation constant - mode #' num2str(mode)])
axis tight
xlabel('Frequency [Hz]')
...
subplot(2,1,2)
for idx=1:3
v_ref=read_from_file(const_model_dataset{idx+3},'vel_dis');
v=read_from_file(LS_model_dataset{idx+3},'vel_dis');
yyv = v ./ v_ref;
semilogx(f,yyv(:,mode),'DisplayName', ['LS model - \rho = ' rho_names{idx} ' \Omega.m'], 'LineStyle','-');hold all

v_ref=read_from_file(const_model_dataset{idx+3},'vel_dis');
v=read_from_file(cigre_model_dataset{idx+3},'vel_dis');
yyv = v ./ v_ref;
semilogx(f,yyv(:,mode),'DisplayName', ['Cigre model - \rho = ' rho_names{idx} ' \Omega.m'], 'LineStyle',':');hold all
end
title(['Normalized phase velocity - mode #' num2str(mode)])
axis tight
xlabel('Frequency [Hz]')
...
set_plot_params()
idx=3;
figname=['SoilModel___rho100___mode' num2str(mode)]
figure('Name',figname)
subplot(2,1,1)
a=read_from_file(const_model_dataset{idx+3},'a_dis');
semilogx(f,a(:,mode),'DisplayName', 'Constant parameters');hold all
a=read_from_file(LS_model_dataset{idx+3},'a_dis');
semilogx(f,a(:,mode),'DisplayName', 'LS model');hold all
a=read_from_file(LS_model_dataset{idx+3},'a_dis');
semilogx(f,a(:,mode),'DisplayName', 'Cigre model');hold all
title(['Attenuation constant - mode #' num2str(mode)])
axis tight
xlabel('Frequency [Hz]')
ylabel('\alpha [Np/m]')
grid on
legend
subplot(2,1,2)
v=read_from_file(const_model_dataset{idx+3},'vel_dis');
semilogx(f,v(:,mode),'DisplayName', 'Constant parameters');hold all
v=read_from_file(LS_model_dataset{idx+3},'vel_dis');
semilogx(f,v(:,mode),'DisplayName', 'LS model');hold all
v=read_from_file(LS_model_dataset{idx+3},'vel_dis');
semilogx(f,v(:,mode),'DisplayName', 'Cigré model');hold all
title(['Phase velocity - mode #' num2str(mode)])
axis tight
xlabel('Frequency [Hz]')
ylabel('\upsilon [m/s]')
grid on
